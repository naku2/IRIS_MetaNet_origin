...
class PreActBasicBlockQ(nn.Module):
    ...
    """
    def __init__(self, wbit_list, abit_list, in_planes, out_planes, stride):
        ...
        # Conv2d = conv2d_quantize_fn(self.wbit_list)
        Conv2d = conv2d_quantize_fn(self.wbit_list, self.abit_list)
        if NN_cfg.bn_type == 'switchbn':
            NormLayer = batchnorm_fn(self.wbit_list)
        elif NN_cfg.bn_type == 'bn': 
            NormLayer = nn.BatchNorm2d
        else:
            raise ValueError("Invalid batchnorm type")

        self.bn0 = NormLayer(in_planes)
        self.act0 = Activate(self.abit_list)
        self.conv0 = Conv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=1, bias=False)
        self.bn1 = NormLayer(out_planes)
        self.act1 = Activate(self.abit_list)
...
class PreActResNet(nn.Module):
    def __init__(self, block, num_units, wbit_list, abit_list, num_classes, expand=5):
        ...
        for stride, channel in zip(strides, channels):
            self.layers.append(block(self.wbit_list, self.abit_list, in_planes, channel, stride=stride))
            in_planes = channel
...
class PreActResNetBottleneck(nn.Module):
    def __init__(self, block, layers, bit_list, num_classes, expand=4):
...
# For ImageNet
def resnet50q(wbit_list, abit_list, num_classes=1000, expand=4):
    return PreActResNetBottleneck(PreActBottleneckQ, [3, 4, 6, 3], bit_list, num_classes=num_classes, expand=expand)

def resnet18q(wbit_list, abit_list, num_classes=1000, expand=4):
    return PreActResNet(PreActBasicBlockQ, [2,2,2,2], wbit_list, abit_list, num_classes=num_classes, expand=expand )